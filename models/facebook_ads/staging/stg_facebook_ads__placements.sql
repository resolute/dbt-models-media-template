...
rename_recast AS (
    ...
    SELECT
        ...
        COALESCE(facebook_entity_account_data.account_name, source_data.account_name) AS account_name,
        source_data.account_name AS account_name_on_date,
        source_data.date,
        source_data.campaign_id,
        COALESCE(facebook_entity_campaign_data.campaign_name, source_data.campaign_name) AS campaign_name,
        source_data.campaign_name AS campaign_name_on_date,
        campaign_type,
        source_data.adset_id,
        COALESCE(facebook_entity_adset_data.adset_name, source_data.adset_name) AS adset_name,
        source_data.adset_name AS adset_name_on_date,
        source_data.ad_id,
        COALESCE(facebook_entity_ad_data.ad_name, source_data.ad_name) AS ad_name,
...
final AS (
    ...
    SELECT 
        ...
        {{ dbt_utils.generate_surrogate_key(['date', 'account_id', 'ad_id', 'impression_device', 'publisher_platform', 'platform_position']) }} AS id,
