...
{%- set cols = adapter.get_columns_in_relation(ref('google_search_ads_360__keywords_conversions_pivoted')) -%}
...
conversion_data AS (
    ...
    SELECT * FROM {{ ref('google_search_ads_360__keywords_conversions_pivoted') }}
...
final AS (
    ...
    SELECT
        ...
        COALESCE(ad_data.engine_account_id, conversion_data.engine_account_id) AS engine_account_id,
        COALESCE(ad_data.engine_account_engine_id, conversion_data.engine_account_engine_id) AS engine_account_engine_id,
        ...
        COALESCE(ad_data.keyword_id, conversion_data.keyword_id) AS keyword_id,
        COALESCE(ad_data.keyword_text, conversion_data.keyword_text) AS keyword_text,
        COALESCE(ad_data.keyword_labels, conversion_data.keyword_labels) AS keyword_labels,
