...
{%- set cols = adapter.get_columns_in_relation(ref('google_ads__keywords_conversions_pivoted')) -%}
...
ad_data AS (
    ...
    SELECT * FROM {{ ref('google_ads__keywords_aggregated') }}

),

conversion_data AS (

    SELECT * FROM {{ ref('google_ads__keywords_conversions_pivoted') }}
