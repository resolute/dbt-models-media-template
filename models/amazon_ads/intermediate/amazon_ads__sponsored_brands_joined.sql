...
ad_data AS (
    ...
    SELECT * FROM {{ ref('stg_amazon_ads__adgroups_hsa') }}

),

campaign_data AS (

    SELECT * FROM {{ ref('stg_amazon_ads__entity_hsa_campaigns') }}
...
join_ad_and_campaign AS (
    ...
    SELECT
        ...
        'sponsoredBrands' AS campaign_type,
        campaign_data.campaign_serving_status,
        campaign_data.campaign_bid_optimization,
        ...
        campaign_data.campaign_spending_policy,
