{%- set relations_list = get_search_paid_files(return_files=false) -%}
...
{%- set cols = adapter.get_columns_in_relation(ref('rollup_search_paid__campaigns')) -%}

WITH

data AS (
  
    SELECT * FROM {{ ref('rollup_search_paid__campaigns') }}
