{{ config(enabled= get_account_ids('amazon ads')|length > 0 is true) }}

{# Get a list of the columns from the upstream model #}
{%- set cols = adapter.get_columns_in_relation(ref('amazon_ads__adgroup_performance_daily')) -%}

WITH

data AS (
  
    SELECT * FROM {{ ref('amazon_ads__adgroup_performance_daily') }}
