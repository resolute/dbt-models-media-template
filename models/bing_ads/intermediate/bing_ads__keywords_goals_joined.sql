...
{%- set cols = adapter.get_columns_in_relation(ref('bing_ads__keywords_goals_pivoted')) -%}
...
ad_data AS (
    ...
    SELECT * FROM {{ ref('bing_ads__keywords_aggregated') }}

),

conversion_data AS (

    SELECT * FROM {{ ref('bing_ads__keywords_goals_pivoted') }}
