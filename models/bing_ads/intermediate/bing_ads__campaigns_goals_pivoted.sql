...
{%- else -%}
    {%- set conversion_fields = var('bing_ads_conversion_metrics', ['assists_conversions', 'conversions', 'value_conversions', 'all_conversions_qualified', 'conversions_qualified', 'view_through_conversions', 'view_through_conversions_qualified', 'view_through_value_conversions','all_value_conversions']) -%}
{%- endif -%}
...
data AS (
    ...
    SELECT * FROM {{ ref('bing_ads__campaigns_goals_pivot_prep') }}
...
final AS (
    ...
    SELECT
        ...
        {%- set conv_name_values = dbt_utils.get_column_values(ref('bing_ads__campaigns_goals_pivot_prep'), 'goal_formatted', default=[]) -%}
