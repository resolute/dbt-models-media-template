...
{%- set cols = adapter.get_columns_in_relation(ref('bing_ads__campaigns_goals_pivoted')) -%}
...
ad_data AS (
    ...
    SELECT * FROM {{ ref('bing_ads__campaigns_aggregated') }}

),

conversion_data AS (

    SELECT * FROM {{ ref('bing_ads__campaigns_goals_pivoted') }}
